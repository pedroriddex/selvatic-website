<script lang="ts">
	...
	const pageKicker = $derived(ONLINE_STORE_ENABLED ? 'Checkout' : 'Tienda online');
	const pageTitle = $derived(ONLINE_STORE_ENABLED ? 'Carrito de compra' : STORE_LOCK_TITLE);
	const pageDescription = $derived(
		ONLINE_STORE_ENABLED
			? 'Revisa tus productos y finaliza el pago con Stripe Checkout.'
		...
		<div class="swiss-grid items-start gap-y-10">
			...
			<div class="col-span-4 md:col-span-3 xl:col-span-5 grid-rail-left">
				...
				<p class="section-note mt-6">
					Si necesitas una pieza o un encargo floral, podemos ayudarte por vía directa mientras terminamos la apertura de la tienda.
				</p>
				<div class="mt-7 flex flex-wrap gap-3">
					<a href="/contacto" class="btn-lime">Solicitar presupuesto</a>
					<a href="/tienda" class="btn-outline">Seguir viendo piezas</a>
				</div>
			</div>
		</div>
	</section>
